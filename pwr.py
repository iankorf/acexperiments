...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='power weight ratio')
	...
	power = [123, 160, 197] # 100 default -> 160, 208, 256 hp
	weight = [907, 1179, 1451] # 1080 default -> 2000, 2600, 3200 lbs

	for p, w in zip(power, weight):

		# RWD - just power mod
		name = f'ace_rwd_{p}_{w}'
		os.system(f'cp -r "{arg.root}"/content/cars/{arg.source} {name}') # copy directory
		os.system(f'rm {name}/data.acd') # force to re-pack data
		desc = f'RWD Miata power={p} weight={w}'
		write_ui(f'{name}/ui/ui_car.json', name, desc, p); # write ui_car.json
		mod_power(f'{name}/data/power.lut', p) # power
		quick_edit(f'{name}/data/car.ini', 'TOTALMASS=1080', f'TOTALMASS={w}')

		# FWD - minimal modifications
		name = f'ace_fwd_{p}_{w}'
		os.system(f'cp -r "{arg.root}"/content/cars/{arg.source} {name}') # copy directory
		os.system(f'rm {name}/data.acd') # force to re-pack data
		desc = f'FWD Miata power={p} weight={w}'
		write_ui(f'{name}/ui/ui_car.json', name, desc, p); # write ui_car.json
		...
		name = f'ace_awd_{p}_{w}'
		os.system(f'cp -r "{arg.root}"/content/cars/{arg.source} {name}') # copy directory
		os.system(f'rm {name}/data.acd') # force to re-pack data
		desc = f'AWD Miata power={p} weight={w}'
		write_ui(f'{name}/ui/ui_car.json', name, desc, p); # write ui_car.json
